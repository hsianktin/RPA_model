...
function dbpath(exp_label,simu_label)
    path = "$simupath/rsa_state_transition_$(exp_label)_$(simu_label).csv"
    return path
end
...
function initialize(exp_label,simu_label)
    ...
    if exp_label == "wt_15mM_salt"
        global exp_data_base=[("wt",0),("wt",0.4),("wt",1),("wt",2.5),("wt",5)]
    ...
    elseif exp_label == "wt_150mM_salt"
        global exp_data_base=[("lwt",0),("lwt",0.4),("lwt",1),("lwt",2.5),("lwt",5)]
        global folds = [0,4,10,25]
    elseif exp_label == "general" # don't use this now
        global exp_data_base=[("wt",0),("wt",0.4),("wt",1),("wt",2.5),("wt",5),("lwt",0),("lwt",0.4),("lwt",1),("lwt",2.5),("lwt",5)]
        global folds = [0,4,10,25]
    end
...
function microstates_plot(k_on,k_off,v_open,v_close,folds)
    ...
    for fold in [0,1,4,10,25]
        ...
        CSV.write("./figs/microstates_$(exp_label)_$fold.csv",temp_df)
        # fig = plot()
...
function diff(EX,μ_X,type="squared errors")
    if type == "squared errors"
        difference = 0.0
        for j in 1800:2400
            # if !(j > 2000.0 && j < 2200.0)
            difference += (EX[j]-μ_X[j])^2
            # end
        end
        return difference
    elseif type == "maximum"
        ...
        return difference
    elseif type == "mixed"
        difference = diff(EX, μ_X, "squared errors")/10 + diff(EX,μ_X,"maximum")
    end
end
