...
using KernelDensity
id = ARGS[1]
function simu_data_acquire(h_on,h_off,D,threshold,l=30)# threshold fixed to 7
    free = 30
    Hairpin = 7
    # initial state
    state = zeros(free+Hairpin+3+Hairpin)
    state[free+Hairpin-l+1:free+Hairpin] .= 1
    # state[1:l] .= 1
    # h_on = 0.1
    # h_off = 0.1
    # D = 3
    ...
    function sample(N)
        ...
        state[free+Hairpin-l+1:free+Hairpin] .= 1
        count = 0
        low_fret_dwell_time = Array{Float64,1}()
        t = 0
        # push!(time,t)
        ...
        while count<N
            ...
            if is_high_fret
                ...
                if find_protein_position(l,state)[1]+(l-1-free) < Hairpin
                    ...
                    # reinitialize state.  next round automatically set is_high_fret to false
                    t=0
                    state = zeros(free+Hairpin+3+Hairpin)
                    state[free+Hairpin-l+1:free+Hairpin] .= 1
                end
        ...
        return low_fret_dwell_time
    end
    waiting_time_low_fret = sample(1e6)
    # waiting_time_low_fret = sample(1000000)
    # writedlm("data_simu/sampled_low_fret_dwell_time_$(l).csv",waiting_time_low_fret,',')
    # histogram(readout)
...
exp_data_base=[("owt",15),("wt",15),("owt",150),("wt",150),("odwh",15),("odwh",150),("dwh",150),("dF",150),("df2",150)]
P_wt = []
P_lwt = []
@showprogress 1 "sampling..." for x in 0:0.1:2.5
    push!(P_wt,probability_density(0.1,0.1,exp(x),7,20))
    push!(P_lwt,probability_density(0.1,0.1,exp(x),7,30))
end
...
df = DataFrame(type = String[], salt_concentration=Float16[], q = Float64[], D_20 = Float64[],D_30 = Float64[])
for exp_label in exp_data_base
    ...
    push!(df,[type,sc,MLE[1],MLE[2],MLE[3]])
end

CSV.write("data_simu/diffusion_report_$(id).csv",df)
