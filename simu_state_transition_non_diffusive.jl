...
if length(Paras) == 10
    ...
    function simulation(T1,T2,Time,Length,Gaps,state_1,state_2,state)
        Simulate(v_open,v_close,0.0,0.0,k_on,k_off,D1,D2,L,l1,l2,T1,Time,Length,Gaps,state_1,state_2,state) # we assume that the state of binding length 20 is an absolute intermediate state. If ABCD(l=30) wants to detach, or bind, it must go through ABC(l=20) state first. D1 and D2 are assumed similarly.
        Simulate(v_open,v_close,0.0,0.0,k_on*fold,k_off,D1,D2,L,l1,l2,T2,Time,Length,Gaps,state_1,state_2,state)
    end
